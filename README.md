...
### ✅ 阶段六：测试与验收 (Quality Assurance & Automated Testing)

**目标**: 构建一套**“写了就不怕改代码”**的现代化测试防护网。拒绝无效测试，专注于高价值的自动化验证。

#### 6.0 测试技术栈选型 (The 2025 Toolchain)
* **测试框架**: **xUnit** (微软官方及开源社区首选，强制隔离，无副作用)。
* **断言库**: **FluentAssertions** (让测试代码读起来像英语句子，如 `result.Should().Be(200)` )。
* **模拟工具 (Mocking)**: **Moq** (老牌经典，文档最全，适合配合 AI 学习)。
* **数据伪造 (Data Gen)**: **Bogus** (不再手动写 "test1@email.com"，自动生成成千上万条真实的假名、地址、职位数据)。
* **集成环境**: **Testcontainers** (抛弃 EF Core InMemory，直接在代码运行时自动启动/销毁真实的 Docker PostgreSQL，保证测试环境与生产环境 100% 一致)。
* **快照测试**: **Verify** (处理复杂 JSON 比对的神器，避免写几十行 Assert)。

#### 6.1 单元测试 (Unit Tests - Core Logic)
**目标**: 验证 `Application` 层业务逻辑的正确性（不涉及数据库）。
- **[ ] 任务 6.1.1 (环境搭建)**:
  - 创建 `MiniHR.UnitTests` 项目。
  - 安装 `xUnit`, `FluentAssertions`, `Moq`, `Bogus`。
- **[ ] 任务 6.1.2 (Service 测试)**:
  - **测试场景**:
    - `CreateEmployee`: 邮箱重复时是否抛出 `DuplicateEmailException`？(利用 Moq 模拟 Repo)。
    - `CreateEmployee`: 正常创建时是否返回正确的 VO？(利用 Bogus 生成随机 DTO)。
    - `GetById`: 找不到 ID 时是否返回 null？

#### 6.2 集成测试 (Integration Tests - Real World)
**目标**: 验证 API 端点到数据库的完整链路（黑盒测试）。
- **[ ] 任务 6.2.1 (容器化环境)**:
  - 创建 `MiniHR.IntegrationTests` 项目。
  - 配置 `Testcontainers`：在测试开始时自动拉取并启动 PostgreSQL 容器。
  - 实现 `CustomWebApplicationFactory`：将 API 的数据库连接字符串替换为测试容器的连接串。
- **[ ] 任务 6.2.2 (Happy Path 测试)**:
  - **流程**: 注册 Admin -> 登录拿 Token -> 创建员工 -> 查询员工。
  - **验证**: 状态码是否为 201/200？返回的 JSON 数据是否与存入的一致？
- **[ ] 任务 6.2.3 (错误处理与鉴权)**:
  - **场景**:
    - 不带 Token 访问 `POST /api/employees` -> 期待 401。
    - 普通用户访问敏感接口 -> 期待 403。
    - 创建重复员工 -> 期待 409 (使用 **Verify** 对比返回的 `ProblemDetails` JSON 结构)。
